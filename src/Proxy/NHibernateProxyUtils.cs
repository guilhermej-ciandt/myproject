        ...
        public static T UnproxyObjectTree<T>(this T persistentObject, ISessionFactory sessionFactory, int maxDepth)
        {
            ...
            for (int i = 0; i < classMetadata.PropertyTypes.Length; i++)
            {
                ...
                if (nhType.IsCollectionType)
                {
                    ...
                    var c = propertyValue as ICollection;
                    var typeArgs = c.GetType().GetGenericArguments();
                    Assembly mscorlib = Assembly.Load("mscorlib");
                    var algTypeGen = mscorlib.GetType("System.Collections.Generic.List`1");
                    var algType = algTypeGen.MakeGenericType(typeArgs);
